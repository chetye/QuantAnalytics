...
namespace marketdata
{
  ...
  double Curve::get_forward(string index, date::FlowFrequency frequency, date::Date start_date, date::Date end_date, 
    date::DayCountConvention dc) const
  {
    double forward = 0;

    double forecast_factor = get_forecast_factor(start_date, end_date);
...
void Interest_Rate_Curve_System::add_curve(std::string type, Interest_Rate_Curve_Ptr curve)
{
  m_curves.insert(std::pair<std::string, Interest_Rate_Curve_Ptr>(type, curve));
}
